...
router.get("/stations", async (req, res) => {
  try {
    const mainCategory = getMainCategoryIdentifierReceived(req.query.mainCategory as string);
    const subCategory = getTableIdentifierReceived(req.query.subCategory as string);
    ...
    if (!mainCategoryIdentifier || !subCategoryIdentifier) {
      res.status(400).json({ error: "Invalid category mapping" });
      return;
    }
    ...
    const result = await pool.request().input("subCategory", subCategory)
      .query(`
...
router.get("/years", async (req, res) => {
  try {
    const mainCategory = getMainCategoryIdentifierReceived(
      req.query.mainCategory as string
    );
    const subCategory = getTableIdentifierReceived(req.query.subCategory as string);
    console.log("✅ Received Section Station mainCategory:", mainCategory);
    console.log("✅ Received Section Station subCategory:", subCategory);
    ...
    console.log(
      "✅ Mapped mainCategoryIdentifier Section Year:",
      mainCategoryIdentifier
    );
    console.log(
      "✅ Mapped subCategoryIdentifier Section Year:",
...
router.get("/semiannuals", async (req, res) => {
  try {
    ...
    console.log(
      "✅ Mapped mainCategoryIdentifier Section Semiannuals:",
      mainCategoryIdentifier
    );
    console.log(
      "✅ Mapped subCategoryIdentifier Section Semiannuals:",
      subCategoryIdentifier
    );
...
router.get("/main-categories", async (req, res) => {
  try {
    ...
    const result = await pool
      .request()
      .query("SELECT DISTINCT mainName FROM dbo.Mcategories");
    res.json(result.recordset);
  } catch (error) {
...
router.get("/sub-categories", async (req, res) => {
  try {
    ...
    if (!mainCategory) {
      res.status(400).json({ error: "Missing mainCategory parameter" });
      return;
    }

    const pool = await connectToDB();
    const result = await pool.request().input("mainCategory", mainCategory as string)
      .query(`
...
router.get("/column-values", async (req, res) => {
  try {
    ...
    if (stationName) request.input("stationName", stationName as string);
    if (year) request.input("year", year as string);
    if (semiannual) request.input("semiannual", semiannual as string);

    const result = await request.query(query);

    res.json(result.recordset.map((row) => row[columnName as string]));
  } catch (error) {
