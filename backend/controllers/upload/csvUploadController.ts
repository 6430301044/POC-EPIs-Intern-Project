...
async function getMainCategoryId(pool: any, mainCategory: string): Promise<string> {
    const result = await pool.request()
        ...
        .query("SELECT main_id FROM dbo.Mcategories WHERE mainName = @mainName");
...
async function getSubCategoryId(pool: any, subCategory: string): Promise<string> {
    const result = await pool.request()
        ...
        .query("SELECT sub_id FROM dbo.SbCategories WHERE subName = @subName");
...
const getTableName = (mainCategory: string, subCategory: string): string => {
    ...
    if (!mainCategoryId || !subCategoryId) {
        throw new Error(`ไม่สามารถแปลงชื่อหมวดหมู่ ${mainCategory} หรือ ${subCategory} เป็นชื่อตาราง`);
    }
    
    // Combine the identifiers to get the table key
    const tableKey = `${mainCategoryId}_${subCategoryId}`;
    console.log("✅ Table Key:", tableKey);
    
    // Look up the table name in the map
    const tableName = tableMap[tableKey];
    if (!tableName) {
        throw new Error(`ไม่พบตารางสำหรับหมวดหมู่ ${mainCategory} และ ${subCategory}`);
    }
    
    return tableName;
};

// Helper function to get column mapping for the selected table
