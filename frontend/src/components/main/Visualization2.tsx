...
export default function Visualization2() {
  ...
  const handleAddYAxisColumn = (column: string) => {
    if (!yAxisColumns.includes(column)) {
      // If column is in secondary axis, remove it from there first
      ...
      console.log('Added Y-axis column:', column, 'Current Y-axis columns:', [...yAxisColumns, column]);
    }
  };
  
  // Handle removing a Y-axis column
  const handleRemoveYAxisColumn = (column: string) => {
    const newColumns = yAxisColumns.filter(col => col !== column);
    setYAxisColumns(newColumns);
    console.log('Removed Y-axis column:', column, 'Current Y-axis columns:', newColumns);
  };

  // Handle adding a secondary Y-axis column
  const handleAddSecondaryYAxisColumn = (column: string) => {
    if (!secondaryYAxisColumns.includes(column)) {
      // If column is in primary axis, remove it from there first
      if (yAxisColumns.includes(column)) {
  ...
  const chartData = useMemo(() => {
    ...
    // ถ้ามีข้อมูลแต่ไม่มีคอลัมน์ที่เลือก ให้แสดงข้อความแนะนำในคอนโซล
    if (yAxisColumns.length === 0 && (!useSecondaryYAxis || secondaryYAxisColumns.length === 0)) {
      console.log('กรุณาเลือกคอลัมน์สำหรับแกน Y อย่างน้อย 1 คอลัมน์');
    }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              ...
              <div>
                ...
                <div className="flex flex-col gap-2">
                  <div className="font-semibold mb-2">Primary Y Axis Columns:</div>
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                    {columns
                      .filter(
                        (col) =>
                          !secondaryYAxisColumns.includes(col)
                      )
                      .map((col) => (
                        <div key={col} className="flex items-center gap-2">
                          <input
                            ...
                            onChange={(e) => {
                              if (e.target.checked) {
                                handleAddYAxisColumn(col);
                              } else {
                                handleRemoveYAxisColumn(col);
                              }
                            }}
                            className="form-checkbox h-4 w-4 text-blue-600"
                          />
                          <label htmlFor={`primary-${col}`} className="text-sm">{col}</label>
                        </div>
                      ))}
                  </div>
                  {yAxisColumns.length > 0 && (
                    <div className="mt-2">
                      <div className="font-semibold mb-1">Selected Primary Y-Axis Columns:</div>
                      <div className="flex flex-wrap gap-2">
                        {yAxisColumns.map((column) => (
                          <div key={column} className="flex items-center gap-1 bg-blue-100 px-2 py-1 rounded">
                            <span className="text-sm">{column}</span>
                            <button
                              onClick={() => handleRemoveYAxisColumn(column)}
                              className="text-red-500 hover:text-red-700 text-sm"
                            >
                              ✕
                            </button>
                            <button
                              onClick={() => moveToSecondaryAxis(column)}
                              className="text-blue-500 hover:text-blue-700 text-sm ml-1"
                              title="Move to Secondary Y-Axis"
                            >
                              →
                            </button>
                          </div>
            ...
            <div className="mb-4">
              ...
              {useSecondaryYAxis && (
                <div className="mt-2 pl-6">
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="font-semibold mb-2">Secondary Y Axis Columns:</div>
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                      {columns
                        .filter(
                          (col) =>
                        ...
                        .map((col) => (
                          <div key={col} className="flex items-center gap-2">
                            <input
                              type="checkbox"
                              id={`secondary-${col}`}
                              checked={secondaryYAxisColumns.includes(col)}
                              onChange={(e) => {
                                if (e.target.checked) {
                                  handleAddSecondaryYAxisColumn(col);
                                } else {
                                  handleRemoveSecondaryYAxisColumn(col);
                                }
                              }}
                              className="form-checkbox h-4 w-4 text-purple-600"
                            />
                            <label htmlFor={`secondary-${col}`} className="text-sm">{col}</label>
                          </div>
                        ))}
5                    </div>
                    {secondaryYAxisColumns.length > 0 && (
                      <div className="mt-2">
                        <div className="font-semibold mb-1">Selected Secondary Y-Axis Columns:</div>
                        <div className="flex flex-wrap gap-2">
                          {secondaryYAxisColumns.map((column) => (
                            <div key={column} className="flex items-center gap-1 bg-purple-100 px-2 py-1 rounded">
                              <span className="text-sm">{column}</span>
                              <button
                                onClick={() => handleRemoveSecondaryYAxisColumn(column)}
                                className="text-red-500 hover:text-red-700 text-sm"
                              >
                                ✕
                              </button>
                              <button
                                onClick={() => moveToPrimaryAxis(column)}
                                className="text-blue-500 hover:text-blue-700 text-sm ml-1"
                                title="Move to Primary Y-Axis"
                              >
                                ←
                              </button>
                            </div>
          ...
          {(data.length > 0 || (showComparison && comparisonDatasets.length > 0)) && (
            <div className="mt-6">
              {yAxisColumns.length === 0 && (!useSecondaryYAxis || secondaryYAxisColumns.length === 0) ? (
                <div className="p-4 bg-yellow-100 text-yellow-800 rounded border border-yellow-300">
                  กรุณาเลือกคอลัมน์สำหรับแกน Y อย่างน้อย 1 คอลัมน์เพื่อแสดงกราฟ
                </div>
              ) : (
                <>
