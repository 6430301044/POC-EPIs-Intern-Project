...
export default function NewsUpload() {
  ...
  const userId = 1;  // สำหรับทดสอบ หรือดึงจาก session หากมี
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      formData.append('content', content); // ส่งข้อความแทนการส่ง JSON
      formData.append('category', category);
      formData.append('Create_by', userId.toString());  // เพิ่มผู้สร้างข่าว
      ...
      const response = await fetch('http://localhost:5000/news/upload', {
        method: 'POST',
        body: formData,
      });
